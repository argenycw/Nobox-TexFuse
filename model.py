'''
Pretrained Models from https://www.robots.ox.ac.uk/~albanie/pytorch-models.html
...
def compose_transforms(meta, resize=256, center_crop=True, override_meta_imsize=False, to_resize=True):
...
def vgg_face_dag(weights_path=None, **kwargs):
    ...
    model = Vgg_face_dag()    
    if weights_path:
        print('Load pretrained model from %s.' % weights_path)
        state_dict = torch.load(weights_path)
        model.load_state_dict(state_dict)
    else:
        raise Exception('Fail to load pretrained model from: %s' % weights_path)
    return model
